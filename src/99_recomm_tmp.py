...
def calc_metrics(unique_labels, set_k, result_table, *DB):
    ...
    for target_label in unique_labels:
        ...
        for i, k in enumerate(set_k):
            ...
            logger.info(f'Tested on: {target_label} with k: {k} || Recall: {RC:.4f}, Hit: {HT:.4f}, MRR: {RR:.4f}')
    ...
    for i, k in enumerate(set_k):
        logger.info(f"For k = {k} RECALL: {RESULT[i][0]:.4f}  HIT: {RESULT[i][1]:.4f}  MRR: {RESULT[i][2]:.4f}")
...
if __name__ == "__main__":
    ...
    result_table = pd.DataFrame(columns = ['Label', 'k', 'Recall@', 'Hit@', 'MRR@'])

    result_metrics = calc_metrics(unique_labels, set_ks, result_table, train_db, test_db)
